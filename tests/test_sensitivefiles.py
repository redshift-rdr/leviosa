...
class TestMutate:
    ...
    async def test_all_levels_including_full_path(self):
        f = make_finder([".env"])
        result = await f.mutate([make_request("http://x.com/a/b/c")], LeviosaContext())
    ...
    async def test_files_have_no_trailing_slash(self):
        f = make_finder([".env"])
        result = await f.mutate([make_request("http://x.com/a")], LeviosaContext())
    ...
    async def test_nested_file_path(self):
        ...
        result = await f.mutate([make_request("http://x.com/app")], LeviosaContext())
    ...
    async def test_query_and_fragment_dropped(self):
        f = make_finder([".env"])
        result = await f.mutate([make_request("http://x.com/a?b=1#f")], LeviosaContext())
        assert result[0].url == "http://x.com/.env"

    async def test_port_and_scheme_preserved(self):
        f = make_finder([".env"])
        result = await f.mutate([make_request("https://x.com:8443/a")], LeviosaContext())
...
class TestAnalyze:
    async def test_404_suppressed(self, capsys):
        f = make_finder()
        await f.analyze([make_response("http://x.com/.env", status=404)], LeviosaContext())
        assert capsys.readouterr().out == ""

    async def test_network_error_suppressed(self, capsys):
        f = make_finder()
        await f.analyze([make_response("http://x.com/.env", status=0)], LeviosaContext())
        assert capsys.readouterr().out == ""

    async def test_200_reported(self, capsys):
        f = make_finder()
        await f.analyze([make_response("http://x.com/.env", status=200)], LeviosaContext())
        assert "[SENSITIVEFILES] 200 GET http://x.com/.env" in capsys.readouterr().out

    async def test_403_reported(self, capsys):
        f = make_finder()
        await f.analyze([make_response("http://x.com/.git/", status=403)], LeviosaContext())
        assert "[SENSITIVEFILES]" in capsys.readouterr().out

    async def test_git_config_flagged(self, capsys):
        f = make_finder()
        body = b"[core]\n\trepositoryformatversion = 0\n\tbare = false\n"
        await f.analyze([make_response("http://x.com/.git/config", status=200, body=body)],
                        LeviosaContext())
    ...
    async def test_private_key_flagged(self, capsys):
        ...
        await f.analyze([make_response("http://x.com/id_rsa", status=200, body=body)],
                        LeviosaContext())
    ...
    async def test_env_secrets_flagged(self, capsys):
        ...
        await f.analyze([make_response("http://x.com/.env", status=200, body=body)],
                        LeviosaContext())
    ...
    async def test_htpasswd_flagged(self, capsys):
        ...
        await f.analyze([make_response("http://x.com/.htpasswd", status=200, body=body)],
                        LeviosaContext())
    ...
    async def test_sql_dump_flagged(self, capsys):
        ...
        await f.analyze([make_response("http://x.com/dump.sql", status=200, body=body)],
                        LeviosaContext())
        assert "exposed: sql-dump" in capsys.readouterr().out

    async def test_plain_body_not_flagged(self, capsys):
        f = make_finder()
        await f.analyze([make_response("http://x.com/.env", status=200, body=b"nothing here")],
                        LeviosaContext())
